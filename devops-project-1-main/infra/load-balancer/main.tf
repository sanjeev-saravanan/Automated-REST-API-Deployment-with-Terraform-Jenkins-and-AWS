...
variable "tf_jenkins_proj_acm_arn" {}
variable "lb_target_group_attachment_port" {}

output "aws_lb_dns_name" {
  value = aws_lb.tf_jenkins_proj_lb.dns_name
}

output "aws_lb_zone_id" {
  value = aws_lb.tf_jenkins_proj_lb.zone_id
}


resource "aws_lb" "tf_jenkins_proj_lb" {
  name               = var.lb_name
...
resource "aws_lb_target_group_attachment" "tf_jenkins_proj_lb_target_group_attachment" {
  target_group_arn = var.lb_target_group_arn
  target_id        = var.ec2_instance_id # Replace with your EC2 instance reference
  port             = var.lb_target_group_attachment_port
}

resource "aws_lb_listener" "tf_jenkins_proj_lb_listner" {
  load_balancer_arn = aws_lb.tf_jenkins_proj_lb.arn
...
resource "aws_lb_listener" "tf_jenkins_proj_lb_https_listner" {
  load_balancer_arn = aws_lb.tf_jenkins_proj_lb.arn
  port              = var.lb_https_listner_port
  protocol          = var.lb_https_listner_protocol
  ssl_policy        = "ELBSecurityPolicy-FS-1-2-Res-2019-08"
  certificate_arn   = var.tf_jenkins_proj_acm_arn
